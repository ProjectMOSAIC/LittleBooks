...
to proportions in this situation.
...
In Section~\ref{sec:bootstrap-t} we will briefly discuss the bootstrap-t interval, a more accurate bootstrap method.  Other methods, such as BCa (bias corrected and accelerated)
...
\coe{Verizon} data set contains repair times for customers in CLEC (competitive) 
...
\noindent
The intervals produced by \code{t.test()} are narrower, do the least to compensate for skew,
