...
\end{tabular}
\end{center}
\DiggingDeeper{The \function{gf\_fitdistr()} within the \pkg{MASS} package facilitates estimation
...
\Rindex{pch option}%
<<tidy=FALSE,fig.keep='last'>>=
gf_dist("norm", mean = 30, sd = sqrt(100 * .3 * .7), 
        fill = ~ (abs(x - 30) > 6), geom = "area") %>%
  gf_dist("binom", size = 100, prob = .3, col = "black", 
          pch = 16)
