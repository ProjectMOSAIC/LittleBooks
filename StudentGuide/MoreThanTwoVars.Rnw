...
xyplot(cesd ~ substance, groups=sex, lwd=2,
  ...
  type='a', data=HELPrct)
gf_line(cesd ~ substance, color = ~ sex, lwd = 2, data = HELPrct)
@
...
gf_point(cesd ~mcs, color = ~sex, data = filter(HELPrct, age == 36), shape = 1) +
  gf_line(gf_lm(mcs, age = 36, sex = "male") ~ mcs)
@
