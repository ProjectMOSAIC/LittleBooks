...
The \function{gf_histogram()} function is used to create a histogram. Here we use the formula interface (as discussed in the \emph{Start Modeling with R} book) to specify that we want a histogram of the CESD scores.
...
gf_dhistogram(~ cesd, bins = 20, data = filter(HELPrct, sex == "female"))
@
\end{center}
...
gf_dhistogram(~ cesd, binwidth = 2, data = filter(HELPrct, sex == "female"))
@
\end{center}
...
gf_dotplot(~ cesd, binwidth = 10, data = filter(HELPrct, sex == "female", homeless == "homeless")) %>%
...
gf_dens(~ cesd, data = filter(HELPrct, sex == "female"))
@
\end{center}
...
<<poly,tidy=FALSE,fig.keep="last",out.width="2.5in">>=
 gf_freqpoly(~ cesd, data = filter(HELPrct, sex == "female"), binwidth = 3.8)
@
\end{center}
