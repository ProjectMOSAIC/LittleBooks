...
% XX NH this could be a place to introduce ggformula (also not sure where to update the textbooks)

Other related resources from Project MOSAIC may be helpful, including an annotated set of examples from the sixth edition of Moore, McCabe and Craig's \emph{Introduction to the Practice of Statistics}\cite{moor:mcca:2007} (see \url{https://nhorton.people.amherst.edu/ips6e}), the second and third editions of the \emph{Statistical Sleuth}\cite{Sleuth2} (see \url{https://nhorton.people.amherst.edu/sleuth}), and \emph{Statistics: Unlocking the Power of Data} by Lock et al (see \url{https://github.com/rpruim/Lock5withR}).
...
library(mosaic)    
# require(mosaic) also can be used to load packages
@
