...
The \function{gf\_plot()} function can be used to display the boxplots for the
...
gf_point(sex ~ length, alpha = .6, cex = 1.4, data = KidsFeet) 
@
\end{center}
...
rtest.stats <- do(500) * diffmean(age ~ shuffle(sex), 
  data = HELPrct)
rtest.stats <- mutate(rtest.stats, diffmeantest = ifelse(diffmean >= test.stat, TRUE, FALSE))
head(rtest.stats, 3)
